...
3. 把字幕翻译成更适合配音的简体中文
4. 用 `edge-tts`、`MiniMax` 或 `CosyVoice` 为每条字幕生成中文配音
...
如果没有 API key，但视频本身带有中文字幕轨，则直接复用中文字幕，不再调用字幕翻译接口；标题、标签和简介也会保留原文。
...
- `final_cn.mp4`：最终导出视频；如果 ffmpeg 当前环境没有 `subtitles` 滤镜，会自动退回成软字幕 MP4
